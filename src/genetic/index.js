...
class Genetic {
	...
	* runIterations() {
	...
	iteration(i) {
		...
		population
			.forEach(solution => { // todo : map? and return score
				solution.score = this.utilities.fitness(solution.choice, this.startingSolution);
	...
	evolve(population) {
		return population.map( (solution, i) => {
			...
			if(i > population.length / 4 ) {
				if(chance > 0.9){ // seed new solution
					return this.mutate(solution);
				}
				if(chance > 0.7){ // cross with new solution
					return this.crossover(solution, this.createSeed());
				}
				if(chance > 0.3){ // cross with another solution
					//const solution2 = Math.floor(Math.random() * population.length)
					const solution2 = Math.floor(Math.abs(Math.random() - Math.random()) * population.length);
